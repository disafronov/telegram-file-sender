...
def main():
    ...
    try:
        response = requests.post(f'https://api.telegram.org/bot{TELEGRAM_BOT_TOKEN}/sendDocument?', data={'chat_id': TELEGRAM_CHAT_ID, 'parse_mode': "MarkdownV2", 'caption': TELEGRAM_CHAT_MESSAGE}, files={'document': open(TELEGRAM_FILE_NAME, 'rb')}, stream=True)
        print(response.text)
